...
if ($filter == 0) {
    switch ($range) {
        case 0 : $query = "Select log.id as logid, log.users_id, u.fullname, log.checkoutRequests_id, log.equipment_id, log.checkoutDate, log.returnDate, log.expectedReturnDate from EqManage.log left join users u on log.users_id = u.id ";
            break;//No range specified
            //and Checkout Date is selected as a method of range selection
        case 1 : $query = $originalQuery."where DATE(checkoutDate) = '$today'";
            break; //When range is today, and Checkout Date is selected as a method of range selection
        case 2 : $query = $originalQuery."where DATE(checkoutDate) = '$yesturday'";
            break; //When range is yesterday, and Checkout Date is selected as a method of range selection
        case 3 : $query = $originalQuery."where DATE(checkoutDate) >= '$weekago'";
            break; //When range is week, and Checkout Date is selected as a method of range selection
        default: $query = null;
    }
};
if ($filter == 1) {
    switch ($range) {
        ...
        case 1 : $query = $originalQuery."where DATE(returnDate) = '$today'";
            break;//When range is today, and Return Date is selected as a method of range selection
        case 2 : $query = $originalQuery."where DATE(returnDate) = '$yesturday'";
            break;//When range is yesterday, and Return Date is selected as a method of range selection
        case 3 : $query = $originalQuery."where DATE(returnDate) >= '$weekago'";
            break;//When range is week, and Return Date is selected as a method of range selection
        default: $query = null;
    }
};
