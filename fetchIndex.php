...
if ($filterCategory == null or $filterCategory == 0) { //if there is no filter
    switch ($sortC) {
        case 1: switch ($sortE) {
            case 1: $executeQuery = $baseQuery.$groupByQuery."ORDER BY C.categoryName asc,E.equipment asc";
    ...
    switch ($sortE) { //Sort by category is ignored as category is filtered
        case 1: $executeQuery = $baseQuery."WHERE C.id=".$filterCategory.$groupByQuery."ORDER BY E.equipment asc";
            break;
        case 2: $executeQuery = $baseQuery."WHERE C.id=".$filterCategory.$groupByQuery."ORDER BY E.equipment desc";
            break;
            break;
    }
} else {
